...
export default function Dashboard() {
  ...
  // ESTADOS CON VALORES POR DEFECTO (Para que no se vea vacío nunca)
  const [price, setPrice] = useState(87240.50); // Precio base
  const [change24h, setChange24h] = useState(1.24);
  const [rsi, setRsi] = useState(45.5);
  const [signal, setSignal] = useState("NEUTRAL");
  const [aiText, setAiText] = useState("Mercado estable. Acumulando volumen para ruptura.");
  const [aiConfidence, setAiConfidence] = useState(88);
  const [candleData, setCandleData] = useState<any[]>([]);
  const [loading, setLoading] = useState(false); // Desactivamos loading inicial para mostrar datos ya

  // DATOS DE RESPALDO (Si la API falla, usamos esto)
  ...
  useEffect(() => {
    // Cargar datos iniciales de respaldo
    ...
    const fetchMarketData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.log("Usando modo offline por error de API"); 
      }
    };
    ...
    const fetchCandles = async () => {
        ...
        } catch (error) { console.log("Usando gráfico offline"); }
    };

    // Ejecutar llamadas reales en segundo plano
    fetchMarketData();
    fetchCandles();

    const intervalPrice = setInterval(fetchMarketData, 5000);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 relative z-20">
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl group hover:border-purple-500/30 transition shadow-lg">
            ...
            <div className="flex items-end gap-3">
                <p className="text-3xl font-mono font-bold text-white">{rsi.toFixed(1)}</p>
            ...
            <div className="w-full bg-slate-800 h-1.5 mt-3 rounded-full overflow-hidden relative">
                <div className={`h-full transition-all duration-500 ${rsi > 70 ? 'bg-red-500' : rsi < 30 ? 'bg-green-500' : 'bg-blue-500'}`} style={{ width: `${rsi}%` }}></div>
            </div>
        </div>
