...
    
    setNotif({ type: 'info', message: `Depósito de $${amt.toFixed(2)} procesado...` });
    setDepositAmount('');
...
              <input required value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="API Key Pública" className="bg-black/30 border border-slate-700 p-3 rounded-lg" />
              <input required type="password" value={secretKey} onChange={(e) => setSecretKey(e.target.value)} placeholder="Secret Key Privada" className="bg-black/30 border border-slate-700 p-3 rounded-lg" />
            </div>
